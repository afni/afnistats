...
#'longer useful. (cli_info: PREFIX,character,1)
...
#' @param verb  Specify verbose level. (cli_info: N,integer,1)
...
#' @param dbgArgs  This option will enable R to save the parameters in a file called
#'.RBA.dbg.AFNI.args in the current directory so that debugging can be
#'performed. (cli_info: ' ',character,0)
...
#'invoke this option so that the program will handle it properly. (cli_info: '',character,0)
#' @param r2z  This option performs Fisher transformation on the response variable
#'(column Y) if it is correlation coefficient. (cli_info: '',character,0)
...
#' \"age vs IQ, age vs weight, IQ vs weight\", where V1, V2, and V3 are three
...
#' @param Y  var_name is used to specify the column name that is designated as
...
#''ROI'. (cli_info: STRING,character,1)
#' @param PDP  Specify the layout of posterior distribution plot (PDP) with nr rows
#'and nc columns among the number of plots. For example, with 16 regions,
#'you can set the number of rows and columns to each. be 4. The region names will be shown in each plot.
#'So label the regions concisely. (cli_info: NDIM,integer,2)
...
for(ii in levels(dat$ROI)) mm[[ii]] = lm(GLM, data=dat[dat$ROI==ii,])
...
if(any(!is.na(EOIq) == TRUE)) for(ii in 1:length(EOIq)) {
   cat(sprintf('===== Summary of region effects under GLM for %s: no adjustment for multiplicity =====', EOIq[ii]),
      file = paste0(outFN, '.txt'), sep = '\n', append=TRUE)
   gg <- sumGLM(ll, EOIq[ii], nR, nn[[ii]]$df, 4)
   cat(capture.output(gg), file = paste0(outFN, '.txt'), sep = '\n', append=TRUE)
   cat('\n', file = paste0(outFN, '.txt'), sep = '\n', append=TRUE)
}
